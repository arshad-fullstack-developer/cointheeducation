...
watch([search, statusFilter, paymentTypeFilter, companyFilter, godownFilter], () => {
    try {
...
const deleteItem = () => {
    ...
    try {
        deleteForm.delete(route('procurements.destroy', itemToDelete.value.id), {
            onSuccess: () => {
                deleteModal.value = false;
                itemToDelete.value = null;
            },
            ...
            <div class="max-w-7xl mx-auto space-y-4">
                ...
                <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-600 via-pink-600 to-orange-500 p-4 text-white shadow-2xl">
                    ...
                    <div class="relative z-10 flex justify-between items-center">
                        <div>
                            <h1 class="text-3xl font-bold mb-1">📦 Procurements</h1>
                            <p class="text-purple-100 text-base">Manage your procurement records with style</p>
                        </div>
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        <!-- Search -->
                        <div class="relative">
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Search Procurements</label>
                            <div class="relative">
                                <input
                                    v-model="search"
                                    type="text"
                ...
                <div class="text-sm text-gray-600">
                    Showing {{ procurements.from || 0 }} to {{ procurements.to || 0 }} of {{ procurements.total || 0 }} procurements
                </div>

                <!-- Enhanced Procurements Table with DESI Design -->
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="procurement in procurements.data" :key="procurement.id" class="hover:bg-gradient-to-r hover:from-purple-50 hover:to-pink-50 transition-all duration-300">
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-3">
                                            <div class="flex-shrink-0">
                                                <div class="w-10 h-10 bg-gradient-to-br from-blue-400 to-purple-400 rounded-xl flex items-center justify-center text-white font-bold text-sm">
                                                    {{ procurement.company.charAt(0).toUpperCase() }}
                                                </div>
                                            </div>
                                            <div class="text-sm font-semibold text-gray-900">{{ procurement.company }}</div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm text-gray-900">{{ procurement.stationary_item?.name }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm text-gray-900">{{ procurement.godown?.name }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm text-gray-900">{{ procurement.salesperson }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm font-semibold text-gray-900">{{ procurement.quantity }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span :class="getStatusBadgeClass(procurement.status)" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold">
                                            {{ procurement.status }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex space-x-3">
                                            ...
                                            <button @click="confirmDelete(procurement)" class="p-3 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-2xl hover:from-red-600 hover:to-pink-600 transition-all duration-300 shadow-lg hover:shadow-xl">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- <div v-if="procurements.links && procurements.links.length > 3" class="px-6 py-4 border-t border-gray-200 bg-gradient-to-r from-purple-50 to-pink-50">
                        <div class="flex items-center justify-between">
                            <div class="text-sm font-semibold text-gray-700">
                                Showing {{ procurements.from }} to {{ procurements.to }} of {{ procurements.total }} procurements
                            </div>
                            <div class="flex space-x-2">
                                <Link
                                    v-for="link in procurements.links"
                                    :key="link.label"
                                    :href="link.url"
                                    :class="[
        ...
        <Modal :show="deleteModal" @close="deleteModal = false">
            <div class="p-6">
                <div class="flex items-center mb-4">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">Delete Procurement</h3>
                        <p class="text-sm text-gray-600">This action cannot be undone</p>
                    </div>
                </div>
                <p class="text-gray-700 mb-6">
                    Are you sure you want to delete the procurement for <strong>{{ itemToDelete?.company }}</strong>? 
                    This will permanently remove the procurement record and all associated data.
                </p>
                <div class="flex justify-end space-x-3">
                    ...
                    >
                        ...
                        <span v-else>Delete Procurement</span>
                    </button>
                </div>
            </div>
        </Modal>
