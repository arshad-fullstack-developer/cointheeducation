...
watch([search, statusFilter, companyFilter, sortBy, sortOrder], () => {
    try {
        router.get('/stationary-items', {
            search: search.value,
...
const deleteItem = () => {
    ...
    try {
        deleteForm.delete(route('stationary-items.destroy', itemToDelete.value.id), {
            onSuccess: () => {
                deleteModal.value = false;
                itemToDelete.value = null;
            },
            ...
            <div class="max-w-7xl mx-auto space-y-4">
                ...
                <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-600 via-pink-600 to-orange-500 p-4 text-white shadow-2xl">
                    ...
                    <div class="relative z-10 flex justify-between items-center">
                        <div>
                            <h1 class="text-3xl font-bold mb-1">📚 Stationary Items</h1>
                            <p class="text-purple-100 text-base">Manage your stationary inventory with style</p>
                        </div>
                        <Link :href="route('stationary-items.create')">
                ...
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        <!-- Search -->
                        <div class="relative">
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Search Items</label>
                            <div class="relative">
                                <input
                                    v-model="search"
                                    type="text"
                ...
                <!-- Enhanced Items Table with DESI Design -->
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="item in stationaryItems.data" :key="item.id" class="hover:bg-gradient-to-r hover:from-purple-50 hover:to-pink-50 transition-all duration-300">
                                    <td class="px-4 py-4">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="text-sm text-gray-600">{{ item.article || '-' }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm text-gray-900">{{ item.company }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm font-semibold text-gray-900">{{ item.current_stock || 0 }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm text-gray-900">{{ formatCurrency(item.unit_cost) }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm text-gray-900">{{ formatCurrency(item.selling_price) }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <span :class="getStatusBadgeClass(item.status)" class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold">
                                            {{ item.status }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex space-x-3">
                                            <Link :href="route('stationary-items.show', item.id)">
                                            ...
                                            <Link :href="route('stationary-items.edit', item.id)">
                                            ...
                                            <button @click="confirmDelete(item)" class="p-3 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-2xl hover:from-red-600 hover:to-pink-600 transition-all duration-300 shadow-lg hover:shadow-xl">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
